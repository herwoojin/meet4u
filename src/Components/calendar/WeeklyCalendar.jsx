...
const WeeklyCalendar = () => {
    ...
    const renderDays = () => {
        ...
        return days.map((date, index) => {
            ...
            return (
                <div key={index} className={`mb-4 bg-white rounded-xl border ${isToday ? 'border-blue-200 ring-1 ring-blue-100' : 'border-gray-200'} p-4 shadow-sm`}>
                    <div className="flex justify-between items-center mb-3 border-b border-gray-100 pb-2">
                        <div className="flex items-center gap-2">
                            <span className={`font-bold text-lg ${index >= 5 ? 'text-red-600' : 'text-gray-900'}`}>{dayName}</span>
                    ...
                    <div className="space-y-2">
                        {dailyMeetings.length === 0 ? (
                            <p className="text-gray-400 text-sm py-2 text-center italic">일정 없음</p>
                        ) : (
